...
private buildRegistrationPayload(): RegistrationRequest {
  ...
  return {
    ...
    taxVatNumber: company.taxId ?? '',
    industryType: company.industry ?? '',
    website: company.website ?? '',

    addressLine1: company.registeredAddress ?? '',
    addressLine2: '',

    emirate: company.emirateState ?? '',
    city: company.city ?? '',
    postalCode: company.postalCode ?? '',

    // Temporary values until Screen 3 is built
    firstName: 'Test',
    lastName: 'Representative',
    designation: 'Authorized Representative',

    officialEmail: company.companyEmail ?? '',
    mobileNumber: company.companyPhone ?? ''
  };
}
